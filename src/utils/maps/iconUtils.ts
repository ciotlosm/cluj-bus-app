...
export function createVehicleIcon(options: VehicleIconOptions): Icon {
  ...
  // Show directional arrow if vehicle is moving
  const showArrow = showDirectionalArrow && speed > 0;
  ...
  const svgContent = `
    <svg width="${iconSize}" height="${iconSize}" viewBox="0 0 ${iconSize} ${iconSize}" xmlns="http://www.w3.org/2000/svg">
      ...
      ${showArrow ? `
        <polygon points="${iconSize/2},${iconSize/2-4} ${iconSize/2-3},${iconSize/2+2} ${iconSize/2+3},${iconSize/2+2}" 
                 fill="#fff" stroke="none"/>
      ` : `
        <circle cx="${iconSize/2}" cy="${iconSize/2}" r="3" fill="#fff"/>
      `}
    </svg>
  `;
